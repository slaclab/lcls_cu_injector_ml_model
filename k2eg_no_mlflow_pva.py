...
k2eg_pvs_to_monitor = ['pva://' + pv for pv in input_parameter_values.keys() if
                       pv not in ['CAMR:IN20:186:R_DIST', 'Pulse_length']]

for pv_name in k2eg_pvs_to_monitor:
    try:
        input_parameter_values[pv_name.replace('pva://', '')] = k2eg_client.get(pv_name, 5.0)["value"]
    except Exception as e:
        logger.warning(f"Failed to get PV {pv_name}: {e}")
        input_parameter_values[pv_name.replace('pva://', '')] = 0.0
